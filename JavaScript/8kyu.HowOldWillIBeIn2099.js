...
in the future such as 2090 or 3044. His parents can't keep up calculating this 
...
function  calculateAge(year_of_birth, current_year) {
    let y = current_year - year_of_birth;
    if (y == 0) return 'You were born this very year!'
    if (y == -1) return 'You will be born in 1 year.'
    if (y == 1) return "You are 1 year old." 
    if (y < 0) return `You will be born in ${Math.abs(y)} years.`
    if (y > 1) return `You are ${Math.abs(y)} years old.` 
}